...
function addSubjectPreviewHandler(event){
...
function addSubjectPreview(subject_preview){
	...
	$(preview_widget).find('#del-btn')[0].onclick = function(click) {
		...
		$('#subjectCreate')[0].removeChild(preview_widget); // Чистим html от превью
		$('#subjectButton')[0].style.display = 'flex' // Открываем кнопку
	}
	$('#subjectCreate').append(preview_widget) // Добавление превью
}
